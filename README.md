...
- With **`--dialect auto`**, the shell runs **one synchronous** dialect detection (client-side **`SHOW`** of **`database_dialect`**, no PROFILE `ExecOptions`). After that (**and** after startup when dialect is explicit), it runs **`SELECT 1`** **in the background** (default query mode, no extra `ExecOptions`) so the first interactive query is less likely to pay full cold-start cost. The prompt is not blocked. Warm-up failures are printed to **stderr** as **`warmup: …`**; the REPL still runs. On shutdown (**Ctrl+C** / signal), warm-up logs nothing if the context is already canceled.
